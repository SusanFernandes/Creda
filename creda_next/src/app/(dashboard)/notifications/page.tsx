...
import { ApiService, connectNotificationWS } from '@/services/api';
...
const Notifications: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Open WebSocket for real-time nudge push
    if (user?.id) {
      const ws = connectNotificationWS(user.id);
      wsRef.current = ws;

      ws.onmessage = (event) => {
        try {
          const msg = JSON.parse(event.data as string);
          if (msg.type === 'nudge' && msg.data) {
            setNudges(prev => [msg.data, ...prev]);
          } else if (msg.type === 'initial_nudges' && Array.isArray(msg.data)) {
            // Backend sends initial unread nudges on connect
            setNudges(prev => {
              const ids = new Set(prev.map((n: any) => n.id));
              const fresh = msg.data.filter((n: any) => !ids.has(n.id));
              return [...fresh, ...prev];
            });
          }
      ...
      ws.onerror = () => {
        // WS failure is non-fatal — HTTP polling already loaded nudges
      };
