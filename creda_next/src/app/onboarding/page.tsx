...
const Onboarding: React.FC = () => {
  ...
  const [form, setForm] = useState({
    ...
    dependents: '0', fire_target_age: '60',
  });
  ...
  const finish = async () => {
    ...
    try {
      await ApiService.upsertProfile({
        ...
        fire_target_age: parseInt(form.fire_target_age) || 60,
        ...
        {step === 2 && (
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div><Label>FIRE Target Age</Label><Input type="number" value={form.fire_target_age} onChange={e => update('fire_target_age', e.target.value)} placeholder="60" className="rounded-xl mt-1" /></div>
              <div className="flex justify-between">
        ...
        {step === 3 && (
          <Card className="text-center">
            <CardContent className="py-16 space-y-6">
              ...
              <Button onClick={() => router.push('/dashboard')} size="lg" className="rounded-xl">Go to Dashboard <ArrowRight className="w-4 h-4 ml-1" /></Button>
            </CardContent>
          </Card>
        )}
      </div>
