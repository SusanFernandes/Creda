...
import { Mic, Loader2, Sparkles, X, Command, ArrowRight } from 'lucide-react';
        ...
        {state !== 'idle' && (
          ...
          >
            <div className="flex items-center justify-between mb-8">
              ...
              <button onClick={cancelInteraction} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
              ...
              {state === 'thinking' && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 px-4 py-3 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">&ldquo;{transcript}&rdquo;</p>
                  </div>
              ...
              {state === 'responding' && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                  <div className="bg-blue-500/5 p-4 rounded-2xl border border-blue-500/10">
                    <p className="text-slate-900 dark:text-white font-medium text-base leading-relaxed">{response}</p>
                  </div>
      ...
      >
        <AnimatePresence mode="wait">
          {state === 'idle' ? (
            ...
            <motion.div key="active" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}>
              {state === 'listening' ? (
