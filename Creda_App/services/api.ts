 ...
 * All calls route through the new Python backend at BACKEND_URL (port 8001).
 */
...
const OFFLINE_CHAT: ChatResponse = {
  ...
  data: {},
  response:
    'Creda is in offline mode. Please start the backend at http://localhost:8001 to enable AI features.',
};
...
export const ApiService = {
  ...
  async chat(req: ChatRequest): Promise<ChatResponse> {
    try {
      return await post<ChatResponse>(`${BACKEND_URL}/chat`, req);
    } catch (err) {
  ...
  async upsertProfile(profile: UserProfile): Promise<any> {
    try {
      return await post(`${BACKEND_URL}/profile/upsert`, profile);
    } catch (err) {
  ...
  async getProfile(userId: string): Promise<UserProfile | null> {
    try {
      return await get<UserProfile>(`${BACKEND_URL}/profile/${userId}`);
    } catch (err) {
  ...
  async getHealthScore(profile: UserProfile): Promise<any> {
    try {
      return await post(`${BACKEND_URL}/agents/money-health`, profile);
    } catch (err) {
  ...
  /** SIP Calculator — routed through chat agent */
  async calculateSIP(req: SIPRequest): Promise<any> {
    try {
      return await post(`${BACKEND_URL}/chat`, {
        message: `Calculate SIP: ₹${req.monthly_amount}/month, ${req.expected_return}% return, ${req.years} years${req.step_up_percent ? `, ${req.step_up_percent}% annual step-up` : ''}`,
        user_id: 'app_user',
      });
    } catch (err) {
  ...
  async firePlanner(req: FIRERequest): Promise<any> {
    try {
      return await post(`${BACKEND_URL}/agents/fire-planner`, req);
    } catch (err) {
  ...
  async taxWizard(req: TaxRequest): Promise<any> {
    try {
      return await post(`${BACKEND_URL}/agents/tax-wizard`, req);
    } catch (err) {
  ...
  async portfolioXray(formData: FormData): Promise<any> {
    try {
      const res = await fetch(`${BACKEND_URL}/portfolio/xray`, {
        method: 'POST',
        body: formData,
      });
  ...
  async couplesPlanner(partner1Id: string, partner2Id: string, goal?: string): Promise<any> {
    try {
      return await post(`${BACKEND_URL}/agents/couples-finance`, {
  ...
  /** RAG knowledge query — routed through chat agent */
  async ragQuery(query: string, userId = 'app_user'): Promise<any> {
    try {
      return await post(`${BACKEND_URL}/chat`, { message: query, user_id: userId });
    } catch (err) {
  ...
  /** Portfolio allocation — routed through chat agent */
  async getPortfolioAllocation(profile: UserProfile): Promise<any> {
    try {
      ...
      return chatRes.data?.allocation ?? chatRes;
    } catch (err) {
  ...
  /** Voice pipeline — send audio, get transcript + AI response + TTS audio back */
  async processVoice(
    audioUri: string,
    languageCode = 'hi',
    userId = 'app_user',
    ...
    try {
      ...
      return {
        ...
        language: json.language ?? languageCode,
      };
    } catch (err) {
  ...
  /** Voice command — send audio, get structured intent back */
  async voiceCommand(
    audioUri: string,
    languageCode = 'en',
